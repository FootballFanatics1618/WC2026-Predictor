...
export async function recalculateGroupStandings(allMatches) {
  ...
  await supabase.from('group_standings').upsert(rows, { onConflict: 'group_name,team' })
  return standings
}
...
export async function propagateGroupWinners(allMatches) {
  ...
  for (const m of r32Matches) {
    ...
    const newA = resolveSlot(m.team_a, slotMap)
    const newB = resolveSlot(m.team_b, slotMap)
...
function resolveSlot(placeholder, slotMap) {
  if (!placeholder) return null
  // Direct match: "Winner A", "Runner-up B"
  if (slotMap[placeholder]) return slotMap[placeholder]
  // Best 3rd: pick first available best 3rd from the listed groups
  if (placeholder.startsWith('Best 3rd')) {
    // Extract group letters from e.g. "Best 3rd (A/B/C/D/F)"
    const match = placeholder.match(/\(([^)]+)\)/)
    if (match) {
      const allowedGroups = match[1].split('/').map(s => s.trim())
      // Find best available 3rd-placed team from those groups
      // We use Best3rd_1..8 order, pick first whose group is in allowedGroups
      for (let i = 1; i <= 8; i++) {
        const key = `Best3rd_${i}`
        // We need to know which group this best3rd came from — store it separately
        // For now, just return Best3rd_1 for the first "Best 3rd" slot encountered
        // A full implementation would track group per best3rd
        if (slotMap[key]) return slotMap[key]
      }
    }
  }
  return null
