...
// Golden Boot lock: 1 hour before tournament kick-off.
// Tournament starts: June 12, 2026 00:30 AM IST = June 11 19:00 UTC
// GB lock = June 11, 2026 23:30 IST = June 11 18:00 UTC
export const GOLDEN_BOOT_LOCK = new Date('2026-06-11T18:00:00Z')
...
export function isGoldenBootLocked(now = new Date()) {
  return now >= GOLDEN_BOOT_LOCK
}
...
export function isMatchPredictionLocked(matchDate, matchTimeET, kickoffUTC, now = new Date()) {
  if (kickoffUTC) {
...
// Uses kickoff_utc if available (DB-authoritative), falls back to ET conversion
export function timeUntilLock(matchDate, matchTimeET, kickoffUTC, now = new Date()) {
  let lockTime
  if (kickoffUTC) {
