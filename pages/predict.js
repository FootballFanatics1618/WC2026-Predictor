  ...
  return (
    <>
      ...
      <div style={{ padding: '1.5rem', maxWidth: '100%', boxSizing: 'border-box' }}>
        ...
        {profile && (
          <div className="card-gold" style={{ maxWidth: '700px', margin: '0 auto 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
            <span style={{ fontWeight: 700, color: 'var(--gold)' }}>👋 {profile.first_name || profile.username}</span>
            <span style={{ color: 'var(--gray-500)', fontSize: '0.875rem' }}>
              🥇 Golden Boot: <strong style={{ color: 'var(--white)' }}>{profile.golden_boot_pick || <span style={{ color: 'var(--danger)' }}>Not set — go to Home!</span>}</strong>
            </span>
          </div>
        )}
